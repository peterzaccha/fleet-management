...
window.livewire.directive("sortable-group", (el, directive, component) => {
    if (directive.modifiers.includes("item-group")) {
        ...
        el.closest("[wire\\:sortable-group]").livewire_sortable.addContainer(
            el
        );
    }
    ...
    sortable.on("sortable:stop", () => {
        setTimeout(() => {
            ...
            el.querySelectorAll("[wire\\:sortable-group\\.item-group]").forEach(
                (el, index) => {
                    let items = [];
                    el.querySelectorAll(
...
window.livewire.directive("sortable", (el, directive, component) => {
    ...
    sortable.on("sortable:stop", () => {
        setTimeout(() => {
            ...
            el.querySelectorAll("[wire\\:sortable\\.item]").forEach(
                (el, index) => {
                    items.push({
                        order: index + 1,
